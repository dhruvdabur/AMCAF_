...
class MppiController:
    ...
    def __init__(
        ...
        self.weights = []
        self.sample_cost_min = None
        self.sample_cost_mean = None
        self.sample_cost_max = None
        self.effective_sample_size = 0.0
        self.last_update_status = "not-run"
    ...
    def update(self, state, time_s):
        ...
        if not self.course:
            ...
            self.sample_cost_max = None
            self.effective_sample_size = 0.0
            self.last_update_status = "course-missing"
            return
        ...
        self.sample_cost_min = float(S.min())
        self.sample_cost_mean = float(S.mean())
        self.sample_cost_max = float(S.max())
        denom = float(np.sum(w ** 2))
        self.effective_sample_size = 0.0 if denom <= 1e-12 else float(1.0 / denom)
    ...
    def get_diagnostics(self):
        """Return a stable diagnostics snapshot for GUI/debug overlays."""
        return {
            "controller_type": "MPPI",
            "solver_success": True,
            "solver_status": self.last_update_status,
            "solver_exception": "",
            "solve_time_ms": None,
            "objective_value": self.sample_cost_mean,
            "sample_cost_min": self.sample_cost_min,
            "sample_cost_mean": self.sample_cost_mean,
            "sample_cost_max": self.sample_cost_max,
            "effective_sample_size": float(self.effective_sample_size),
            "num_samples_K": int(self.K),
            "horizon_step_T": int(self.T),
            "target_steer_rad": float(self.target_steer_rad),
            "target_accel_mps2": float(self.target_accel_mps2),
            "target_yaw_rate_rps": float(self.target_yaw_rate_rps),
            "target_speed_mps": float(self.target_speed_mps),
            "has_optimal_trajectory": self.optimal_trajectory is not None,
            "sampled_trajectory_count": len(self.sampled_trajectories),
        }
