...
Model Predictive Control (MPC) controller with a kinematic bicycle model.
The optimizer predicts the vehicle as a single front-steered body with
state (x, y, yaw, velocity) and control input (steer, acceleration).
"""
...
class MPCController1:
    def __init__(
        ...
        tw_default        = self._sw * np.array([2., 2., 2., 4.]) 
        ...
        self.tracking_error_m = 0.0
        self.tracking_error_percent = 0.0
        self.speed_error_percent = 0.0
    ...
    def _get_reference_trajectory(self, x0: float, y0: float,
        ...
        max_search_idx = len(self._ext_x) if self.closed_loop else n
        ...
        best_idx = min(best_idx, max_search_idx - 1)
        ...
        for k in range(self.T + 1):
            if not self.closed_loop and best_idx >= n - 1:
    ...
    def update(self, state, time_s: float) -> None:
        ...
        self._update_error_metrics(px, py, vel)

        dist_to_goal = float("inf")
        if self._n_course > 0 and not self.closed_loop:
    ...
    def _update_error_metrics(self, px: float, py: float, vel: float) -> None:
        ref_x = float(self._current_ref[0, 0])
        ref_y = float(self._current_ref[1, 0])
        ref_vel = float(self._current_ref[3, 0])

        self.tracking_error_m = float(np.hypot(px - ref_x, py - ref_y))
        if self._course_length_m > 1e-9:
            self.tracking_error_percent = (
                self.tracking_error_m / self._course_length_m * 100.0
            )
        else:
            self.tracking_error_percent = 0.0

        if abs(ref_vel) > 1e-9:
            self.speed_error_percent = abs(vel - ref_vel) / abs(ref_vel) * 100.0
        else:
            self.speed_error_percent = 0.0
    ...
    def draw(self, axes, elems) -> None:
        info_text = (
            f"Tracking error: {self.tracking_error_m:.2f} m "
            f"({self.tracking_error_percent:.2f}%)\n"
            f"Speed error: {self.speed_error_percent:.1f}%"
        )
        elems.append(
            axes.text(
                0.02,
