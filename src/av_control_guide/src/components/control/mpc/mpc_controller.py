...
class MPCController:
    ...
    def __init__(
        ...
        self.max_steer = self.max_steer = max_steer_abs
        ...
        self.last_solver_status   = "not-run"
        self.last_solver_exception = ""
    ...
    def update(self, state, time_s: float) -> None:
        if not self.course:
            ...
            self.solver_success      = False
            self.last_objective_value = None
            self.last_solver_status   = "course-missing"
            self.last_solver_exception = ""
            return
        ...
        if self._n_course > 0:
            ...
            if self._stopped:
                ...
                self.last_solver_status   = "stopped-latched"
                self.last_solver_exception = ""
                return
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                self.solver_success = True
                self.last_solver_status = "ok"
                self.last_solver_exception = ""
            except Exception:
                u0 = np.zeros((2, 1))
                self.solver_success = False
                self.last_solver_status = "exception"
                self.last_solver_exception = "solver failed during make_step"
        self.solve_time_ms = (time.perf_counter() - t_start) * 1e3

        if self.solver_success:
            try:
                self.last_objective_value = float(np.asarray(self._mpc.opt_aux_num["_aux", self.T, 0, "cost"]).flat[0])
            except Exception:
                self.last_objective_value = None
        else:
            self.last_objective_value = None
