...
class CubicSplineCourse:
    ...
    def __init__(self, x_ref_points, y_ref_points, target_speed_kmph, resolution=0.1,
                 color='gold', road_width_m=6.0):
        """
        Constructor
        ...
        color: Color of drawing course centerline/setpoints
        road_width_m: Width[m] of the road drawn around the course centerline
        """
    ...
    def draw(self, axes, elems):
        """
        Function to draw course as a road with the setpoint path as centerline
        axes: Axes object of figure
        elems: List of plot objects
        """

        half_width_m = self.road_width_m / 2.0
        x_array = np.array(self.x_array)
        y_array = np.array(self.y_array)
        yaw_array = np.array(self.yaw_array)

        left_x = x_array - np.sin(yaw_array) * half_width_m
        left_y = y_array + np.cos(yaw_array) * half_width_m
        right_x = x_array + np.sin(yaw_array) * half_width_m
        right_y = y_array - np.cos(yaw_array) * half_width_m

        road_patch = axes.fill(
        ...
        elems.append(road_patch)

        left_edge, = axes.plot(left_x, left_y, color='white', linewidth=1.2, zorder=1)
        right_edge, = axes.plot(right_x, right_y, color='white', linewidth=1.2, zorder=1)
        centerline, = axes.plot(
            self.x_array,
            self.y_array,
            color=self.color,
            linewidth=1.4,
        ...
        elems.extend([left_edge, right_edge, centerline, setpoints])
