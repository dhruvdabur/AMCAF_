...
from ..config.ellipse_static import VELOCITY_SCALE
from ..ui import noop
...
class EllipseStaticTuning:
    ...
    def create_pid_panel(self):
        ...
        cv2.createTrackbar(
            'R_SAFE x100',
            PID_WINDOW,
            int(round(self.config.cbf_r_safe * CBF_R_SAFE_SCALE)),
            500,
            noop,
        )
        cv2.createTrackbar(
    ...
    def read_panel_values(self):
        ...
        values = {
            ...
            / VELOCITY_SCALE,
            'cbf_r_safe': cv2.getTrackbarPos('R_SAFE x100', PID_WINDOW)
            / CBF_R_SAFE_SCALE,
        ...
        values['cbf_r_safe'] = max(0.01, values['cbf_r_safe'])
        ...
        self.controller.apply_tuning_values(
            {
                ...
                'cbf_r_safe': payload.get('cbf_r_safe', self.config.cbf_r_safe),
