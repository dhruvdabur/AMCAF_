# hil.nodes

ROS 2 node implementations for HIL executables.

| File | Purpose |
| --- | --- |
| `ellipse_static_node.py` | Closed elliptical road ArUco follower node and its `main()` function. |
| `straight_static_node.py` | Straight-road ArUco follower node and its `main()` function. |

Files here should focus on ROS wiring and high-level behavior. Geometry,
configuration, drawing, metrics, and QoS helpers live in sibling packages.
