#!/usr/bin/env python3
"""Follow a virtual camera-frame track using an ArUco marker on the vehicle."""
...
class PIDController:
    """Small PID controller for image-space steering error."""
...
def parse_args(args=None):
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Detect ArUco DICT_4X4 marker ID 0 on /image_raw, draw a virtual '
            'track, and publish bounded RC commands to follow it.'
        )
    )
    ...
    parser.add_argument(
        '--controller-mode',
        choices=('pid', 'pid_velocity', 'pid_velocity_cbf'),
        default='pid',
...
class ArucoTrackFollower(Node):
    ...
    def throttle_to_pwm(
        ...
        throttle = self.config.forward_pwm + speed_delta
        throttle = bounded(
            throttle,
        ...
        if self.config.controller_mode == 'pid_velocity_cbf':
            scale = self.cbf_safety_scale(
                center,
                lateral_error_px,
                heading_error_rad,
            )
            throttle = self.config.neutral_throttle_pwm + (
                throttle - self.config.neutral_throttle_pwm
            ) * scale
            self.cbf_scale = scale
            self.cbf_active = scale < 0.999
        else:
            self.cbf_scale = 1.0
            self.cbf_active = False
