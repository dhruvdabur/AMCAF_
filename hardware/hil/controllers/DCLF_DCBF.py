"""QP-based control Lyapunov and barrier function safety filter with elliptical barriers."""
...
@dataclass
class DCLF_DCBFCar:
    """Bicycle-model car state consumed by the elliptical DCLF-DCBF QP."""
...
VehicleState = DCLF_DCBFCar


@dataclass
class DCLF_DCBFPointObstacle:
    """Obstacle center and optional dimensions consumed by the DCLF-DCBF QP."""
...
@dataclass
class DCLF_DCBFConfig:
    """Tunable constants for the elliptical DCLF-DCBF safety filter."""
...
class DCLF_DCBFSafetyFilter:
    """Filter nominal commands through a DCLF-DCBF QP with elliptical barriers."""

    def __init__(self, config: Optional[DCLF_DCBFConfig] = None):
        self.config = config or DCLF_DCBFConfig()
        self.counter = 0
    ...
    def solve(self, car, obstacles, accel_ref, delta_ref):
        ...
        try:
            ...
            raise RuntimeError(
                'cvxpy is required to solve DCLF-DCBF QP controls.'
            ) from exc
        ...
        clf_slack = None
        if getattr(cfg, 'clf_alpha', 0.0) > 0.0:
            cte = float(car.prev_y_error)
            he = float(car.prev_psi_error)
            v = float(car.v)
            dt = 0.02
            k_psi = 10.0
            V_k = 0.5 * (cte**2 + k_psi * he**2)
            lhs_clf_delta = k_psi * he * (v / max(1e-6, float(cfg.wheelbase))) * dt
            rhs_clf = -float(cfg.clf_alpha) * V_k - cte * v * he * dt
            
            clf_slack = cp.Variable(nonneg=True)
            constraints.append(lhs_clf_delta * delta <= rhs_clf + clf_slack)
        ...
        if clf_slack is not None:
            objective_terms.append(
                float(getattr(cfg, 'clf_slack_weight', 500.0)) * cp.square(clf_slack)
            )
